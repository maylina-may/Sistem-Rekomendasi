...
Pada bagian Data Preparation ini, melakukan pemberihan data dengan menghapus semua baris yang memiliki missing value menggunakan `df.dropna()`. Dan setelah itu dilakukan dengan pengecekan ulang meggunakan `df.isna().sum() unutk memastikan tidak ada lagi missing value nya dalam dataset.
"""
...
# Membuat DataFrame TF-IDF dengan dengan baris berupa judul film dan kolom berupa jenis genre
...
Teknik cosine similarity di peroleh dari perhitungan TF-IDF matrix untuk mengukur kemiripan antar genre dalam film
"""
